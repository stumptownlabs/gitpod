...
export class TypeORMUserDBImpl implements UserDB {
  ...
  public async findGitpodTokensOfUser(
    userId: string,
    tokenHash: string
  ): Promise<GitpodToken | undefined> {
    const repo = await this.getGitpodTokenRepo();
    const qBuilder = repo
      .createQueryBuilder("gitpodToken")
      .leftJoinAndSelect("gitpodToken.user", "user");
  ...
  public async findTokenForIdentity(
    ...
    if (tokenEntries.length > 1) {
      log.warn(
        `Found more than one active token for ${identity.authProviderId}.`,
        { identity }
      );
    }
    ...
    return tokenEntries
      .sort((a, b) =>
        `${a.token.updateDate}`.localeCompare(`${b.token.updateDate}`)
      )
      .reverse()[0]?.token;
  }
