...
export class SnapshotState1635953505010 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    if (!(await columnExists(queryRunner, TABLE_NAME, "state"))) {
      await queryRunner.query(
        `ALTER TABLE ${TABLE_NAME} ADD COLUMN state char(32) NOT NULL DEFAULT 'available'`
      );
    }
    await queryRunner.query(
      `UPDATE ${TABLE_NAME} SET state = 'available' WHERE state = ''`
    );

    if (!(await columnExists(queryRunner, TABLE_NAME, "message"))) {
      await queryRunner.query(
        `ALTER TABLE ${TABLE_NAME} ADD COLUMN message varchar(255) NOT NULL DEFAULT ''`
      );
    }

    if (!(await indexExists(queryRunner, TABLE_NAME, INDEX_NAME))) {
      await queryRunner.query(
        `CREATE INDEX ${INDEX_NAME} ON ${TABLE_NAME} (state)`
      );
    }
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<any> {
    await queryRunner.query(
      `ALTER TABLE ${TABLE_NAME} DROP COLUMN availableTime`
    );
