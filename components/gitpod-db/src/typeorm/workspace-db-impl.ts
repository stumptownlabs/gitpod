...
export abstract class AbstractTypeORMWorkspaceDBImpl implements WorkspaceDB {
  ...
  protected async getPrebuildInfoRepo(): Promise<Repository<DBPrebuildInfo>> {
    return await (
      await this.getManager()
    ).getRepository<DBPrebuildInfo>(DBPrebuildInfo);
  }
  ...
  public async updateSnapshot(
    snapshot: DeepPartial<Snapshot> & Pick<Snapshot, "id">
  ): Promise<void> {
    const snapshots = await this.getSnapshotRepo();
  ...
  async findInstancesByPhaseAndRegion(
    phase: string,
    region: string
  ): Promise<WorkspaceInstance[]> {
  ...
  async findPrebuiltWorkspacesByProject(
    ...
    query.andWhere(
      `pi.prebuildId in (${filteredIds.map((id) => `'${id}'`).join(", ")})`
    );
