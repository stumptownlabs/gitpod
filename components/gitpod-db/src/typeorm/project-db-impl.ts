...
function toProjectEnvVar(
  envVarWithValue: ProjectEnvVarWithValue
): ProjectEnvVar {
...
export class ProjectDBImpl implements ProjectDB {
  ...
  protected async getProjectEnvVarRepo(): Promise<Repository<DBProjectEnvVar>> {
    return (await this.getEntityManager()).getRepository<DBProjectEnvVar>(
      DBProjectEnvVar
    );
  }

  protected async getProjectInfoRepo(): Promise<Repository<DBProjectInfo>> {
    return (await this.getEntityManager()).getRepository<DBProjectInfo>(
      DBProjectInfo
    );
  }
  ...
  public async findProjectsByCloneUrls(
    ...
    const teamIds = Array.from(
      new Set(projects.map((p) => p.teamId).filter((id) => !!id))
    );
    ...
    for (const project of projects) {
      result.push({
        ...project,
        teamOwners: teamIdsAndOwners
          .filter((i) => i.teamId === project.teamId)
          .map((i) => i.owner),
      });
    }
  ...
  public async getProjectEnvironmentVariables(
    projectId: string
  ...
  public async getProjectEnvironmentVariableById(
    variableId: string
  ...
  public async deleteProjectEnvironmentVariable(
    variableId: string
  ): Promise<void> {
    const envVarRepo = await this.getProjectEnvVarRepo();
    const envVarWithValue = await envVarRepo.findOne({
    ...
    if (!envVarWithValue) {
      throw new Error(
        "A environment variable with this name could not be found for this project"
      );
    }
  ...
  public async findCachedProjectOverview(
    projectId: string
  ...
  public async storeCachedProjectOverview(
    projectId: string,
